...
- No external assets required by default (files = off). Everything renders with
  rectangles, circles, and text. If you want to experiment with your own art,
  flip USE_ASSETS=True and place images in assets/smb3/*.png (filenames noted
  below). The game runs fine without any files.
...
- Names in comments are purely for reference to gameplay styles. No third‑party
  assets are included. If you choose to use your own images, ensure you have
...
# Toggle external art. If True, attempt to load images from assets/smb3/.
USE_ASSETS = False
ASSET_DIR = os.path.join("assets", "smb3")
...
def load_image(name: str) -> Optional[pygame.Surface]:
    """Attempt to load an image from ASSET_DIR. Returns None on failure."""
...
def try_load_assets():
    if not USE_ASSETS:
        return
    # These filenames are suggestions; replace with your own if desired.
    candidates = {
        "player": "player.png",
        "goomba": "enemy_goomba.png",
        "boss":   "boss_boom.png",
        "ground": "tile_ground.png",
        "brick":  "tile_brick.png",
        "question": "tile_question.png",
        "spike":  "tile_spike.png",
        "goal":   "flag.png",
        "over_node": "over_node.png",
    }
    for key, fname in candidates.items():
        IMAGES[key] = load_image(fname)
...
class Player:
    ...
    def draw(self, surface, camera_x):
        ...
        if USE_ASSETS and IMAGES.get("player"):
            img = IMAGES["player"]
            if self.facing < 0:
...
class Walker:
    ...
    def update(self, solids: List[pygame.Rect]):
        ...
        has_floor = False
        for s in solids:
            if ahead.bottom <= s.top and ahead.centerx >= s.left and ahead.centerx <= s.right and abs(ahead.bottom - s.top) <= TILE:
                # rough heuristic; we will do a downward probe instead
                pass
        # Downward probe
    ...
    def draw(self, surface, camera_x):
        r = self.rect.move(-camera_x, 0)
        if USE_ASSETS and IMAGES.get("goomba"):
            img = IMAGES["goomba"]
            if self.vx < 0:
                img = pygame.transform.flip(img, True, False)
            surface.blit(img, img.get_rect(center=r.center))
...
class BoomStyleBoss:
    ...
    def draw(self, surface, camera_x):
        r = self.rect().move(-camera_x, 0)
        if USE_ASSETS and IMAGES.get("boss"):
            img = IMAGES["boss"]
            surface.blit(img, img.get_rect(center=r.center))
        else:
...
class Level:
    ...
    def draw_tiles(self, surface, camera_x):
        for r in self.solids:
            ...
            if USE_ASSETS and IMAGES.get("ground"):
                surface.blit(IMAGES["ground"], rr)
            else:
        ...
        for r in self.hazards:
            rr = r.move(-camera_x, 0)
            if USE_ASSETS and IMAGES.get("spike"):
                surface.blit(IMAGES["spike"], rr)
            else:
                pygame.draw.polygon(surface, RED, [(rr.left, rr.bottom), (rr.centerx, rr.top), (rr.right, rr.bottom)])

        if self.goal:
            rr = self.goal.move(-camera_x, 0)
            if USE_ASSETS and IMAGES.get("goal"):
                surface.blit(IMAGES["goal"], rr)
            else:
...
class OverworldMap:
    ...
    def draw(self, surface, font, progress_text):
        ...
        # nodes
        for i, n in enumerate(self.nodes):
            if USE_ASSETS and IMAGES.get("over_node"):
                img = IMAGES["over_node"]
                r = img.get_rect(center=n.pos)
                surface.blit(img, r)
            else:
...
class Game:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("SMB3-Style Engine (Single File)")
